...
static inline unsigned int map_bin_to_size(unsigned int bin) {
	return quick_pow2(bin + 2);
}

static inline unsigned int map_size_to_bin(unsigned int size) {
	return quick_log2(size) - 2;
}
